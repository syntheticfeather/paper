...
把语法分析的结果和抽象语法树一次读取，一并输出。

只写抽象语法树，然后根据抽象语法树计算值

```bash
# 首先进入目录
cd compile/practice/task2

# 编译
flex lexer.l
gcc -o a lex.yy.c rdparser2.c -I.
./a.exe
```
...
完成CallParams、RelExp、Stmt的递归下降函数,首先要完成所有下级递归函数
...
gcc -Iinclude lex.yy.c rdparser.c ast.c ast_print.c -o a

# 启动程序
./a.exe

# 每次程序输入结束后输出Ctrl+Z输入EOF，结束词法分析。
Ctrl+Z
```

其中Block 和 ArraySubscripts可直接使用
past rd_block();
past rd_array_subscripts();


✅ 正确做法：循环迭代（而非递归右结合）
要正确处理左递归文法，应该用 迭代方式累积左操作数：
MUL
